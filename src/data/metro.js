...
const METRO_LINES = {
  ...
  /* Z4线一期北段：寨上—北塘，全长23.7km，10站（2026-01-18开通）
     ...
     path：额外路径控制点，用于让polyline贴合实际线路走向（海岸弧线等），不绘制站点圆点
     坐标来源：高德卫星图+路网实描（GCJ-02） */
  z4north: {
    name: 'Z4线北段',
    ...
    /* 路径控制点（插入在指定站点idx之后），用于拟合海岸/道路弧线，不画圆点 */
    path: [
      /* 汉沽医院→中心渔港段：沿汉南路东行再转向东南 */
      { after: 1, points: [
        [39.2200, 117.8380],
        [39.2155, 117.8455]
      ]},
      /* 中心渔港→玉砂道段：沿海岸大弧线转向西南（原硬折角→平滑弧形） */
      { after: 2, points: [
        [39.2050, 117.8480],
        [39.2000, 117.8440],
        [39.1950, 117.8380],
